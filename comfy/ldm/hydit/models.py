...
from .attn_layers import Attention, CrossAttention, apply_rotary_emb
...
class HunYuanDiTBlock(nn.Module):
    ...
    def _forward(self, x, c=None, text_states=None, freq_cis_img=None, skip=None,transformer_options=None):
        ...
        if (('blocks',transformer_options['layer_index']) in attn2_replace_patch.keys()):
            b, s1, c = x.shape
            _, s2, c = text_states.shape   
            
            q=self.attn2.q_proj(self.norm3(x)).view(b, s1,  self.attn2.num_heads,  self.attn2.head_dim)
            q = self.attn2.q_norm(q)    
            if freq_cis_img is not None:
                qq, _ = apply_rotary_emb(q, None, freq_cis_img)
                assert qq.shape == q.shape, f'qq: {qq.shape}, q: {q.shape}'
                q = qq
                
            # kv  = self.attn2.kv_proj(text_states).view(b, s2, 2, self.attn2.num_heads* self.attn2.head_dim) 
            kv  = self.attn2.kv_proj(text_states).view(b, s2, 2, self.attn2.num_heads, self.attn2.head_dim)
            k, v= kv.unbind(dim=2) # [b, s, h, d]
            k = self.attn2.k_norm(k)
            
            q=q.view(b, s1,  self.attn2.num_heads* self.attn2.head_dim)
            x = x+attn2_replace_patch[('blocks', transformer_options['layer_index'])](q,k,v,extra_options)        ### patch infer
    ...
    def forward(self, x, c=None, text_states=None, freq_cis_img=None, skip=None,transformer_options=None):
        if self.gradient_checkpointing and self.training:
            return checkpoint.checkpoint(self._forward, x, c, text_states, freq_cis_img, skip)
        return self._forward(x, c, text_states, freq_cis_img, skip,transformer_options)
...
class HunYuanDiT(nn.Module):
    ...
    def forward(self,
        ...
        transformer_options['mlp_t5']=self.mlp_t5
        transformer_options['dtype']=self.dtype
        transformer_options['text_embedding_padding']=self.text_embedding_padding
        transformer_options['text_len']=self.text_len
        transformer_options['text_len_t5']=self.text_len_t5
        ...
        for layer, block in enumerate(self.blocks):
            transformer_options['layer_index']=layer
            if layer > self.depth // 2:
                if controls is not None:
                    skip = skips.pop() + controls.pop().to(dtype=x.dtype)
                else:
                    skip = skips.pop()
                x = block(x, c, text_states, freqs_cis_img, skip,transformer_options)   # (N, L, D)
            else:
                x = block(x, c, text_states, freqs_cis_img,None,transformer_options)         # (N, L, D)
